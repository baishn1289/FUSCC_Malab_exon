...

{
  ...
  table(result_df_hyper$formula)
  
  result_logistic_hyper <- result_df_hyper %>% 
  ...
  save(result_df_hyper,problematic_genes_hyper,
...
{  
  ...
  filtered_df_hyper <- filtered_df_hyper %>% 
                        head(20)
...
{  
  ...
  p_mid <- filtered_df_hyper %>% 
            ...
            scale_x_continuous(trans = 'log10',
                               breaks = c(1,2,4,8),
                               labels = scales::number_format(accuracy = 0.1)) +
            coord_cartesian(ylim = c(1,21), xlim = c(1,8)) +
            geom_vline(xintercept = c(1,mean(filtered_df_hyper %>% filter(or>1) %>% pull(or))), linetype = c(1,2)) +  
            theme(axis.line.y = element_blank(), axis.ticks.y = element_blank(),
                  axis.text.y = element_blank(), axis.title.y = element_blank(),
                  axis.text.x = element_text(color = "black"))
