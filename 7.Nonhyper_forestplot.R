...

{
  ...
  table(result_df_nonhyper$formula)
  
  result_logistic_nonhyper <- result_df_nonhyper %>% 
  ...
  save(result_df_nonhyper,problematic_genes_nonhyper,
...
{  #nonhyper_EO_low
  ...
  filtered_df_nonhyper <- df_nonhyper %>% 
                          filter(adjPval <= adjustPval_threshold) %>%
                          filter(LOCRC_freq > EOCRC_freq) %>%
  ...
  filtered_df_nonhyper <-filtered_df_nonhyper %>% 
                          head(20)
  ...
  p_mid <- filtered_df_nonhyper %>% 
            ...
            labs(x = "Odds Ratio") +
             scale_x_continuous(trans = 'log10',breaks = c(0.1,0.5,1.0),
                                labels = scales::number_format(accuracy = 0.01)) +
             coord_cartesian(ylim = c(1, 2),xlim = c(0.1,1.0)) +
...

{
  ...
  p_mid <- filtered_df_nonhyper %>% 
            ...
            geom_errorbarh(aes(xmin = ci.low, xmax = ci.up), height = 0.3) +
            labs(x = "Odds Ratio") +
            scale_x_continuous(trans = 'log10', breaks = c(1.0,2.0),
                               labels = scales::number_format(accuracy = 0.01)) +
  ...
  plot_grid(p_hyper_high,
            p_hyper_low,
            p_nonhyper_high,
            p_nonhyper_low,
            ncol = 1,rel_heights = c(26,6))
