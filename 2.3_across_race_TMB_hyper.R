...
result_df_race_hyper <- data.table::data.table(
  Hugo_Symbol = NA,
  EOCRC_total_case= NA,
  LOCRC_total_case= NA,
  EOCRC = NA, 
...
for (i in c("Asian Pacific lslander","Black","White")){
  ...
  write_csv(result_df,file = paste('./table/2.2_logsitic',i,'original_TMBhyper.csv',sep = '_'))
  ...
  write_csv(result_df_modified,file = paste('./table/2.2_logsitic',i,'modified_TMBhyper.csv',sep = '_'))
  
  result_df_race_hyper <-  rbind(result_df_race_hyper,result_df_modified)
}
  
  result_df_race_hyper <- na.omit(result_df_race_hyper)
  
  write_csv(result_df_race_hyper,file = './table/2.2_logsitic_result_df_race_TMB_hyper.csv')
  ...
  write.csv(result_df_race_Qtest,file = './table/2.2_result_df_race_Qtest_hyper.csv')
  ...
  write.csv(result_race_chi,file = './table/2.2_result_race_chi_hyper.csv')
  ...
  {#数据整合
    ...
    Asia_data_hyper <- read.csv('./table/2.2_logsitic_Asian Pacific lslander_modified_TMBhyper.csv') %>%
      select(Hugo_Symbol, EOCRC_freq, LOCRC_freq, adjPval, OR, ci.up, ci.low) %>%
      rename_with(~ paste("Asia", ., sep = "_"), -Hugo_Symbol)
    
    Black_data_hyper <- read.csv('./table/2.2_logsitic_Black_modified_TMBhyper.csv')%>% 
      select(Hugo_Symbol,EOCRC_freq,LOCRC_freq,adjPval,OR,ci.up,ci.low)%>%
      rename_with(~ paste("Blcak", ., sep = "_"), -Hugo_Symbol)
    
    White_data_hyper <- read.csv('./table/2.2_logsitic_White_modified_TMBhyper.csv')%>% 
      select(Hugo_Symbol,EOCRC_freq,LOCRC_freq,adjPval,OR,ci.up,ci.low)%>%
    ...
    write_csv(Race_data_all_hyper,file = './table/2.2_Race_data_all_hyper.csv')
  }
