...
library(ggthemes)
  ...
  Gene_list_country_LO <- merge( setDT(clin.LOCRC@clinical.data[, .(Country, Panel)]), 
...
for (i in common_genes) {
  ...
  write.csv(mutation_table_LO,file = './table/5.0_LOCRC_Country_commongenes_mutation_table_LO.csv')
  ...
  pdf(paste(dir,"/4_EOCRC_LOCRC_common_genes_mainplot.pdf", sep = ""), width = 10, height = 5.5)
  plot_common_EO_LO
  dev.off()
...
{  #supplementary plot
  ...
  Common_country_EO
  ggsave(paste(dir,"/4_EOCRC_common_genes_supplementary.pdf",sep = ""), 
