...
library(ggthemes)
  ...
  Gene_list_country_EO <- merge( setDT(clin.EOCRC@clinical.data[, .(Country, Panel)]), 
...
generate_country_frequency <- function(countries, maf,jiyin){
  ...
  Gene_panels_samples <- maf@clinical.data %>% 
    filter(Panel %in% gene_panel_map[[jiyin]])
...
for (i in common_genes) {
  ...
  write.csv(mutation_table_EO,file = './table/4.0_EOCRC_Country_commongenes_mutation_table_EO.csv')